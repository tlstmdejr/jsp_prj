					...
					<table class="table table-hover">
						...
						<tbody>
							<c:if test="${empty files }">
								<tr>
									<td colspan="4">업로드된파일이없습니다</td>
								</tr>
							</c:if>
							<%-- 
							<c:forEach ar="file" items="${files }" varStatus="i">
								<tr>
									<td><c:out value="${i.count }"/></td>
										<td><c:out value="${file.name }"/></td>
									<%-- 	EL은 GETTER호출하는데 LENGTH는 GETTER가아니므로
									호출할수없어 에러가 발생 
							...
							for (int i = 0; i < files.length; i++) {
								...
								if(urlName!=null){
									//파일명이 queryString에 영향을 주는 문자열이 있다면 download.jsp로 보내지지않으므로
									//encoding해야한다
							...
							<tr>
								<td><%=i + 1%></td>
								<td><a href="${CommonURL}/day1212/download.jsp?fileName=<%=urlName%>"><%=file.getName()%></td>
								<td><%=file.length()%></td>
